...
function cargarCategorias() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var nav = $('#categoryNav');
            nav.html('<span class="category-pill active" data-idcategoria="" onclick="filtrarPorPill(this,\'\')">Todas</span>');

            $.each(data, function(index, categoria) {
                select.append('<option value="' + categoria.idcategoria + '">' + categoria.nombre + '</option>');
                nav.append('<span class="category-pill" data-idcategoria="' + categoria.idcategoria + '" onclick="filtrarPorPill(this,\'' + categoria.idcategoria + '\')">' + categoria.nombre + '</span>');
            });
...
function mostrarFormularioPago() {
    ...
    $('#nombreCliente, #emailCliente, #telefonoCliente, #calleCliente, #numeroCliente, #ciudadCliente, #provinciaCliente, #codigoPostalCliente').on('input', function() {
        ...
        if (currentLength >= maxLength * 0.9) {
            $counter.css('color', 'var(--pharma-danger)');
        } else if (currentLength >= maxLength * 0.7) {
            $counter.css('color', 'var(--pharma-warning)');
        } else {
            $counter.css('color', 'var(--pharma-text-secondary)');
        }
    });
...
$(document).ready(function() {
    ...
    $('#categoryFilter, #marcaFilter, #priceFilter, #sortFilter').change(function() {
        if (this.id === 'categoryFilter') {
            ...
            $('#categoryNav .category-pill').removeClass('active');
            $('#categoryNav .category-pill[data-idcategoria="' + idcategoria + '"]').addClass('active');
        }
        cargarProductos(1);
    });
});
...
function clearFilters() {
    $('#categoryNav .category-pill').removeClass('active');
    $('#categoryNav .category-pill[data-idcategoria=""]').addClass('active');
    limpiarFiltros();
}

// Filtra los productos al hacer clic en una categoría de la barra de navegación
function filtrarPorPill(pill, idcategoria) {
    $('#categoryNav .category-pill').removeClass('active');
    $(pill).addClass('active');
    $('#categoryFilter').val(idcategoria);
    cargarProductos(1);
}
